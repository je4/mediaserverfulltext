...
import (
	...
	"github.com/je4/mediaserverfulltext/v2/configs"
	"github.com/je4/mediaserverfulltext/v2/pkg/service"
...
func main() {
	...
	if *cfg != "" {
		...
		cfgFile = "mediaserverfulltext.toml"
	}
	conf := &MediaserverFulltextConfig{
		LocalAddr:   "localhost:8443",
		Concurrency: 3,
		TikaTimeout: config.Duration(15 * time.Second),
	}
	if err := LoadMediaserverFulltextConfig(cfgFS, cfgFile, conf); err != nil {
	...
	srv, err := service.NewActionService(actionDispatcherClient, host, uint32(port), conf.TikaFulltextURl, time.Duration(conf.TikaTimeout), conf.Concurrency, conf.QueueSize, time.Duration(conf.ResolverNotFoundTimeout), vfs, dbClient, logger)
	if err != nil {
